function [ OptimalValue, DownlinkRate_PerUser1, DownlinkRate_PerUser2, W1, W2, alpha, beta, vartheta1, vartheta2 ] = Get_optSolutionPerIteration( Pbs, Hk, Gm, Rate_Threshold, Conven, Fixed_timegroup_assignment, W1_current, W2_current, alpha_current, beta_current, vartheta1_current, vartheta2_current, Init )
...
% Constraints
alpha_bar_next = sdpvar(1, 1, 'full', 'real');



% cons = [cons, alpha_next>=1];
cons = [cons, alpha_next>=1];
cons = [cons, alpha_next<=10];
cons = [cons, cone([1 0.5*(alpha_next-alpha_bar_next)], 0.5*(alpha_next+alpha_bar_next)) ];
    
if (~Conven)    
    beta_bar_next = sdpvar(1, 1, 'full', 'real');
%     cons = [cons, beta_next>=2];
    cons = [cons, beta_next>=1];
    cons = [cons, beta_next<=10];
    cons = [cons, cone([1 0.5*(beta_next-beta_bar_next)], 0.5*(beta_next+beta_bar_next)) ];
else
...
cons = [cons, alpha_bar_next + beta_bar_next <= 1];
...
%         cons = [cons, vartheta_next(k,g)>0];
        ...
        cons = [cons, cone([Sum_vec 0.5*(Theta2_next(m)-vartheta2_next(m))], 0.5*(Theta2_next(m)+vartheta2_next(m)) )];
...
cons = [cons, cone([W1_next(:); 0.5*(alpha_tilde_next-1)], 0.5*(alpha_tilde_next+1)) ];
